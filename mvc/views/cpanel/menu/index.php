                    ...
                    <tbody>
                        <?php foreach($data['datas'] as $key => $val){?>
                            <tr class="even<?= $val['id'] ?> pointer">
                                ...
                                <td class=""><input type="number" onkeyup="Sorting(this, <?= $val['id'] ?>)"  id="sort<?= $val['id'] ?>" data-control="<?= $data['template'] ?>" value="<?= $val['sort'] ?>"></td>
                                <td>
                            ...
                            <?php if(isset($val['children']) && $val['children'] != NULL){?>
                                <?php foreach($val['children'] as $key_child => $val_child){?>
                                    <tr class="even<?= $val_child['id'] ?> pointer">
                                        ...
                                        <td class=""><input type="number" onkeyup="Sorting(this, <?= $val_child['id'] ?>)"  id="sort<?= $val_child['id'] ?>" data-control="<?= $data['template'] ?>" value="<?= $val_child['sort'] ?>"></td>
                                        <td>
...
<script>
    ...
    function Sorting(__This, id){
        const control = __This.getAttribute('data-control');
        const value = __This.value;
        let xhr = new XMLHttpRequest();
       xhr.open('POST',control + '/sort');
